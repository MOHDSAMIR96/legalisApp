...
export default function ClientRegister({ navigation }) {
  ...
  const showRegisterView = () => {
    if (!registerBtnDisplayed) {
      ...
      Animated.timing(animateScroll, { toValue: 3, duration: 500 , useNativeDriver: true, }).start()
    }
    else if (registerBtnDisplayed) {
      stateNewRegisterBtnDisplayed(0)
      Animated.timing(animateScroll, { toValue: 0, duration: 500 , useNativeDriver: true, }).start()
    }
      ...
      <View style={{ flex: 2, flexDirection: 'row', backgroundColor: "#4170f9" }}>
        ...
        <View style={{ flex: 10, flexDirection: 'row', backgroundColor: "#4170f9" }}>
          <ScrollView horizontal={true}>
            {avatarArr.map(
              function (item, index) {
                return <TouchableHighlight key={index} onPress={() => setNewRegisterAvatar(index)}><Image key={index} source={item} style={{ width: 70, height: 70 }} /></TouchableHighlight>
              })}
          </ScrollView>
        </View>
